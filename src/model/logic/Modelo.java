...
public class Modelo {
	 ...
	 * 
	 * @param tipo de estructura: 1 = Arreglo dinamico, 2 = Lista encadenada default
	...
	public Peliculas verificarDetalles(String[] detalle) {
		try {
			...
			if (detalle[16].compareTo("") != 0 && detalle[16] != null) {
				// Cosas a verificar:
				if (detalle[16].equals(""))
					return null; // Descarta los datos sin nombre
				String nombre = detalle[16].trim(); // Nombre de la pelicula
				if (detalle[0].equals(""))
					return null; // Descarta los datos sin id
				Integer id = Integer.parseInt(detalle[0]); // identificacion
				if (detalle[17].equals(""))
					return null; // Descarta los datos sin puntuacion
				Double puntuacion = Double.parseDouble(detalle[17]); // Puntuacion
				if (detalle[18].equals(""))
					return null; // Descarta los datos sin votos
				...
				if (fecha.equals(""))
					return null; // Descarta los datos sin fecha
	 ...
	 *         encuentra el director NOTA: En la ultima casilla de devuelve el
	 *         promedio de puntaje de todas las peliculas
	 */
	 ...
	 * @param Tipo       de clasificación: 1= Descendente (Mejores) - 2= Ascendente
	 *                   (Peores)
	 * @param Criterio   a comparar
	 * @param SubArreglo con datos, si es null se utilizan todos
	 * @return String[] lista de las peliculas en el orden especificado null si hay
	 *         problemas NOTA: ultimos 2 datos dan el vote count average y
	 *         puntuacion average
	 */
	public String[] rankingPeliculas(Integer cant, Integer tipo, Integer criterio, Lista<Peliculas> subDatos) {
		...
		if (aux != null && aux.size() > 0) {
			...
			if (criterio == null)
				...
				ShellSort.sort(pelisPuntuacion, comp);
	...
	public String[] conocerActor(String actorAConocer) {
		...
		for (int i = 1; i <= datos.size(); i++) {
			for (String actor : datos.get(i).darActores()) {
				if (actor.equals(actorAConocer))
					peliculasDeActor.add(datos.get(i));
			}
		}
		...
		int numPel = 0;
		for (Peliculas pelicula : peliculasDeActor) {
			if (dirQueNoSon.contains(pelicula.darDirector()))
				continue;
			else {
				int cant = 0;
				for (Peliculas peli : peliculasDeActor) {
					if (peli.darDirector().equals(pelicula.darDirector()))
						cant++;
				}
				if (cant > numPel) {
					numPel = cant;
